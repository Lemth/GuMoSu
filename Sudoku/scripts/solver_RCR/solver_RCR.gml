...
for(var j=0;j<9;j++) {
	for(var i=0;i<9;i++) {
		if(ds_list_size(argument0[| j*9+i])==1) {
			ds_list_add(singles_value,ds_list_find_value(argument0[| j*9+i],0));
		} else {
			ds_list_add(multiples_id,argument0[| j*9+i]);
		}
	}
	while(ds_list_size(singles_value)>0) {
		for(var i=0;i<ds_list_size(multiples_id);i++) {
			status=ds_list_delete_value(multiples_id[| i],singles_value[| 0]);
			if(status==true) { status2=true; }
		}
		ds_list_delete(singles_value,0);
	}
...
for(var j=0;j<9;j++) {
	for(var i=0;i<9;i++) {
		if(ds_list_size(argument0[| i*9+j])==1) {
			ds_list_add(singles_value,ds_list_find_value(argument0[| i*9+j],0));
		} else {
...
for(var j=0;j<9;j++) {
	for(var i=0;i<9;i++) { //PREVENT DOUBLE CHECKS!
		if(ds_list_size(argument0[| 18*floor(j/3)-2*(i mod 3)+3*(j+i)])==1) {
			ds_list_add(singles_value,ds_list_find_value(argument0[| 18*floor(j/3)-2*(i mod 3)+3*(j+i)],0));
		} else {
