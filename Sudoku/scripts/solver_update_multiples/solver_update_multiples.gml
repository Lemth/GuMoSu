...
var value=ds_list_find_value(argument0[| argument1],0);

//Check row (move through colummns)
for(var i=0;i<9;i++) {
	if(i!=column) {
		if(ds_list_size(argument0[| row*9+i])>1) {
			size=ds_list_delete_value(argument0[| row*9+i],value);
			if(size==1) { status=max(status,solver_update_multiples(argument0,row*9+i)); }
...
for(var i=0;i<9;i++) {
	if(i!=row) {
		if(ds_list_size(argument0[| i*9+column])>1) {
			size=ds_list_delete_value(argument0[| i*9+column],value);
			if(size==1) { status=max(status,solver_update_multiples(argument0,i*9+column)); }
			else if(size>1) { status=1; }
			else if(size==0) { return -1; }
		}
	}
...
for(var i=0;i<9;i++) { //prevent double check:
	...
	if(column!=(square mod 9) && row!=floor(square/9)) {
		if(ds_list_size(argument0[| square])>1) {
			size=ds_list_delete_value(argument0[| square],value);
			if(size==1) { status=max(status,solver_update_multiples(argument0,square)); }
			else if(size>1) { status=1; }
			else if(size<=0) { return -1; }
		}
	}
