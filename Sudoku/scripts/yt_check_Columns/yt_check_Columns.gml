...
show_debug_message("We got here^2!");
...
for(var i=0;i<9;i++) {
	...
	for(var j=0;j<9;j++) {
		...
		for(var k=0;k<9;k++) {
			...
			if(ds_list_find_value(ds_map_find_value(sudoku_grid[# i,j],"possible"),k) == 0) {
				sum[| k]++;
				place[| k]=j;
			}
		}
	...
	for (var k=0;k<9;k++) {
		if(sum[| k]==1) {
			...
			ds_map_replace(sudoku_grid[# i,place[| k]],"number",k+1);
			// https://www.youtube.com/watch?v=Nc7eCcTkzxg 15:55
			ds_map_replace(sudoku_grid[# i,place[| k]],"solvable",0);
			unsolved--;
			
			// https://www.youtube.com/watch?v=Nc7eCcTkzxg 16:37
			yt_update_Sudoku(sudoku_grid,i,place[| k]);
			yt_update_Boxes(sudoku_grid,i,place[| k]);
